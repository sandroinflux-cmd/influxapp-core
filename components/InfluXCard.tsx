...
function GridDealCard({ deal }: { deal: any }) {
  ...
  return (
    ...
    >
      <div className="absolute inset-0 bg-[#020502] border border-white/10 rounded-[45px] p-6 flex flex-col justify-between shadow-inner relative overflow-hidden group/deal" style={{ backfaceVisibility: 'hidden', WebkitBackfaceVisibility: 'hidden' }}>
...
interface InfluXCardProps {
  ...
  disableRotation?: boolean; // 👈 ვამატებთ ამას
}
...
export default function InfluXCard({ profile, liveDeals = [], deal, children, disableRotation = false }: InfluXCardProps) {
  ...
  // 🛡️ თუ disableRotation არის, მაშინ ბრუნვა ნულია
  ...
  return (
    <div className="relative w-full flex flex-col items-center">
      <div className="relative w-full" onMouseMove={(e) => { if(!disableRotation) { const rect = e.currentTarget.getBoundingClientRect(); x.set(e.clientX - (rect.left + rect.width / 2)); y.set(e.clientY - (rect.top + rect.height / 2)) } }} onMouseLeave={() => { x.set(0); y.set(0) }} onDoubleClick={() => setIsFlipped(!isFlipped)}>
        ...
        {/* 📏 შეცვლილია: w-full და aspect-ratio ფიქსირებული პიქსელების ნაცვლად */}
        <motion.div 
        ...
        >
          {/* FRONT */}
          <div className="absolute inset-0 bg-[#040d08]/98 border-2 border-emerald-500/10 rounded-[45px] p-6 pt-10 flex flex-col items-center shadow-2xl overflow-hidden" style={{ backfaceVisibility: "hidden" }}>
          ...
          {/* BACK */}
          <div className="absolute inset-0 bg-[#010201] border-2 border-white/10 rounded-[45px] overflow-hidden" style={{ backfaceVisibility: "hidden", transform: "rotateY(180deg)" }}>
