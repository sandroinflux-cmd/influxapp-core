...
const menuItems = [
  // ... (აქ დატოვეთ ზუსტად ის menuItems მასივი თავისი SVG-ებით, რაც თქვენს კოდში გქონდათ)
  ...
  { name: 'Deals', path: '/dashboard/influencer/deals', color: '#3b82f6', icon: <svg width="20" height="20" viewBox="0 0 24 24" fill="none" xmlns="http://www.w3.org/2000/svg"><path d="M16 3H21V8M8 21H3V16M21 3L12 12M3 21L12 12" stroke="currentColor" strokeWidth="1.5" strokeLinecap="round" strokeLinejoin="round"/></svg> },
...
// 🚀 ვიღებთ status-ს Layout-იდან
export default function Sidebar({ status = 'approved' }: { status?: string }) {
      ...
      <div className="mt-auto pt-10 border-t border-white/5 flex flex-col items-center">
        <div className={`flex items-center ${isExpanded ? 'gap-5 w-full' : 'justify-center'}`}>
          <div className="h-10 w-10 rounded-[15px] bg-[#040d08] border border-emerald-500/10 flex items-center justify-center relative flex-shrink-0">
              <div className={`h-1.5 w-1.5 rounded-full ${status === 'pending' ? 'bg-yellow-500 shadow-[0_0_8px_#eab308]' : status === 'suspended' ? 'bg-red-500 shadow-[0_0_8px_#ef4444]' : 'bg-emerald-500 shadow-[0_0_8px_#10b981]'}`} />
          </div>
          {isExpanded && (
            <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} transition={{ duration: 0.1 }} className="flex flex-col">
              <span className="text-[9px] text-white font-black uppercase tracking-widest italic leading-none whitespace-nowrap">Auth: {status}</span>
