...
function PaymentContent() {
  ...
  const handleScanSuccess = async (decodedText: string) => {
    try {
      ...
      if (activeTokenRaw) {
        ...
        const { data: p } = await supabase.from('partnerships').select('*, deals(*)').eq('influencer_id', influencerId).eq('brand_id', brandId).single();
        setActiveDeal(p || { user_discount_pct: 0, influencer_id: influencerId, brand_id: brandId });
      } else {
        setActiveDeal({ user_discount_pct: 0, influencer_cut_pct: 0, brand_id: brandId });
      }
      setStep('amount');
    } catch (err) { 
      alert("QR Signal Interrupted."); 
    }
  }

  // 💸 გადახდის ინიცირება (ბანკის ინტეგრაციისთვის)
  ...
  if (loading) return (
    <div className="flex flex-col items-center gap-6">
      ...
      <p className="text-emerald-500 font-black animate-pulse tracking-[0.5em] uppercase italic text-[10px]">Linking to Bank...</p>
    </div>
  )
    ...
    <div className="w-full max-w-md mx-auto relative z-10">
      {step === 'scan' && (
        <div className="space-y-10 animate-in fade-in zoom-in duration-700">
          ...
          <button onClick={() => router.back()} className="w-full py-4 text-[9px] font-black text-gray-700 uppercase tracking-[0.5em] italic hover:text-white transition-colors">[ Abort Transmission ]</button>
        </div>
      )}

      {step === 'amount' && (
        <AmountModal deal={activeDeal} onConfirm={handlePaymentConfirm} onCancel={() => { localStorage.removeItem('matrix_active_token'); router.push('/wallet') }} />
      )}
    </div>
  )
}
