...
export default function BrandSettingsPage() {
  ...
  const handleCommit = async () => {
    ...
    try {
      const { error } = await supabase
        .from('profiles')
        .update({ iban, full_name: brandName, avatar_url: avatarUrl, sector, address, bio })
        .eq('id', brandId)
  ...
  // ⚡ ტერმინალის ბმულის გენერაცია
  const terminalUrl = typeof window !== 'undefined' 
    ? `${window.location.origin}/dashboard/cashier/monitor?brand=${brandId}`
    : ''

  const copyTerminalLink = () => {
    navigator.clipboard.writeText(terminalUrl)
    alert("TERMINAL LINK SECURED 📟")
  }
      ...
      <div className="max-w-6xl space-y-16">
        <section className="grid grid-cols-1 lg:grid-cols-2 gap-16 border-t border-white/5 pt-16">
          ...
          <div className="space-y-10">
            ...
            <div className="flex items-center gap-10">
              <label className="relative cursor-pointer group/avatar">
                <motion.div 
                  ...
                  className="h-32 w-32 rounded-[40px] bg-[#020502] border-2 border-blue-500/20 flex items-center justify-center overflow-hidden transition-all group-hover/avatar:border-blue-500 relative"
                >
            ...
            <div className="space-y-6">
              ...
              <div className="space-y-2">
                <label className="text-[9px] text-gray-600 uppercase font-black tracking-widest italic ml-4">HQ Physical Location</label>
                <input type="text" value={address} onChange={(e) => setAddress(e.target.value)} placeholder="STREET, CITY" className="w-full bg-white/[0.02] border border-white/5 rounded-2xl p-6 text-sm font-black focus:border-blue-500/40 outline-none transition-all uppercase italic tracking-[2px]" />
              </div>
          ...
          {/* RIGHT SIDE: Payment & Terminal Protocol */}
            ...
            <div className="bg-blue-600/[0.03] border border-blue-500/20 rounded-[40px] p-10 relative overflow-hidden shadow-2xl">
              <div className="flex items-center justify-between mb-8">
                <div>
                  ...
                  <p className="text-[9px] text-gray-600 uppercase font-black tracking-widest italic">Magic link for cashier monitor access.</p>
                </div>
              ...
              <div className="space-y-4">
                <div className="bg-black/40 border border-white/5 rounded-2xl p-5 flex items-center justify-between gap-4">
                  <div className="overflow-hidden">
                    ...
                    <p className="text-[10px] text-blue-500/60 font-black italic truncate uppercase">{terminalUrl || 'Syncing Node...'}</p>
                  </div>
                  <button 
                    onClick={copyTerminalLink}
                    className="shrink-0 bg-blue-600 hover:bg-blue-500 text-white px-6 py-3 rounded-xl text-[9px] font-black uppercase tracking-widest italic transition-all active:scale-95"
                  >
                    Copy Link
                  </button>
                </div>
                <p className="px-4 text-[8px] text-gray-700 font-black uppercase italic leading-relaxed tracking-wider">
                  ⚠️ share this link only with your authorized cashier nodes. no password required for monitor access.
                </p>
              </div>
            </div>
        ...
        <div className="pt-16 border-t border-white/5 flex justify-end items-center gap-10">
          ...
          <button 
            ...
            className="px-20 py-6 bg-white text-black rounded-full text-[12px] font-black uppercase tracking-[0.6em] hover:bg-blue-600 hover:text-white transition-all duration-500 italic shadow-2xl disabled:opacity-50"
          >
            {isSaving ? 'Syncing...' : 'Commit Identity'}
          </button>
        </div>
      </div>
    </main>
