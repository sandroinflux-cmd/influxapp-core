...
export default function BrandDashboard() {
  return (
    <div className="p-4 md:p-10 lg:p-14 space-y-10 max-w-7xl mx-auto pb-24">
      ...
      {/* 📊 Main Metrics Grid: Sapphire Aesthetic */}
        ...
        {metrics.map((item, i) => (
          ...
          >
            ...
            <div className="flex items-baseline gap-2">
              <h2 className="text-4xl font-black tracking-tighter italic group-hover:text-white transition-colors">{item.value}</h2>
      ...
      {/* 📈 Central Activity Block */}
      <div className="grid grid-cols-1 lg:grid-cols-12 gap-8">
        
        {/* Campaign Pulse */}
        <div className="lg:col-span-8 bg-[#03080c] border border-white/5 rounded-[45px] p-10 relative overflow-hidden">
          <div className="flex justify-between items-center mb-10">
            <span className="text-[10px] font-black text-blue-500 uppercase tracking-[0.4em] italic">Campaign Velocity</span>
            <button className="text-[8px] font-black text-gray-600 uppercase tracking-widest hover:text-white transition-colors underline decoration-blue-500/50 underline-offset-8">Expand Report</button>
          </div>
          ...
          <div className="h-64 w-full flex items-end gap-3 justify-between px-4">
             {[...Array(12)].map((_, i) => (
               ...
               >
                 <div className="absolute -top-6 left-1/2 -translate-x-1/2 opacity-0 group-hover:opacity-100 transition-opacity text-[8px] font-black text-blue-400">Node_{i+1}</div>
               </motion.div>
             ))}
          </div>
        </div>
        ...
        <div className="lg:col-span-4 space-y-6">
          <button className="w-full bg-blue-600 hover:bg-blue-500 text-white py-10 rounded-[35px] font-black text-[11px] tracking-[0.5em] uppercase transition-all shadow-[0_0_30px_rgba(59,130,246,0.2)] active:scale-95 flex flex-col items-center gap-4">
             <span className="text-3xl italic leading-none">+</span>
             Launch Campaign
          </button>
          <button className="w-full bg-white/5 hover:bg-white/10 border border-white/10 text-white py-8 rounded-[35px] font-black text-[9px] tracking-[0.4em] uppercase transition-all flex flex-col items-center gap-3">
