...
export default function MyDealsPage() {
  ...
  const finalizePush = async () => {
    ...
    try {
      const total = splittingDeal.deals?.totalShare || 0
      const influencerCut = total - userDiscount

      const { error } = await supabase
        .from('partnerships')
        .update({ 
          user_discount_pct: userDiscount,
            ...
            <div className="bg-[#020502] border border-emerald-500/30 rounded-[40px] p-10 w-full max-w-md shadow-2xl text-center italic font-black uppercase">
              <div className="space-y-12">
                <div>
                  ...
                  <p className="text-5xl">{splittingDeal.deals?.totalShare || 0}%</p>
                </div>
                <div className="space-y-6 text-left">
                  <div className="flex justify-between text-[9px] tracking-widest leading-none">
                    ...
                    <span className="text-white/30 font-black uppercase">YOU: {(splittingDeal.deals?.totalShare || 0) - userDiscount}%</span>
                  </div>
                  <input type="range" min="1" max={(splittingDeal.deals?.totalShare || 1) - 1} value={userDiscount} onChange={(e) => setUserDiscount(parseInt(e.target.value))} className="w-full h-1 bg-white/10 rounded-full appearance-none cursor-pointer accent-emerald-500" />
                </div>
                  ...
                  <div className="opacity-0 group-hover:opacity-100 transition-all duration-500 transform translate-y-4 group-hover:translate-y-0 absolute w-full z-20">
                    <button 
                      onClick={(e) => { 
                        ...
                        setUserDiscount(item.user_discount_pct || Math.floor((item.deals?.totalShare || 0) * 0.7)); 
                      }} 
